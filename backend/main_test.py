...
def get_attendance(
    ...
    page_size: int = 20,
    pivot_date: Optional[str] = None  # 👈 keyset anchor
):
    ...
    if isinstance(page, str) and page.lower() == "last":
        last_offset = max((total_records // page_size) * page_size - page_size, 0)
        data_query = f"""
            SELECT * FROM (
            ...
            ORDER BY punch_date
        """
        paginated_params = params + [page_size, last_offset]
        page = (total_records + page_size - 1) // page_size  # set currentPage
    elif isinstance(page, int) and page > 5 and pivot_date:
        # Keyset pagination starting after pivot_date
        filters.append("punch_date > %s")
        params.append(pivot_date)
        base_query_with_keyset = base_query + " AND punch_date > %s"

        data_query = f"""
            SELECT emp_id, emp_name, gender, dept_name, distance_from_office, punch_date, attendance_status, punch_in_time, punch_out_time
            {base_query_with_keyset}
            ORDER BY punch_date
            LIMIT %s
        """
        paginated_params = params + [page_size]
    else:
        # Offset pagination
        data_query = f"""
            SELECT emp_id, emp_name, gender, dept_name, distance_from_office, punch_date, attendance_status, punch_in_time, punch_out_time
