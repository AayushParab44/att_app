...
const AttendanceFilterPage = () => {
  ...
  const fetchData = async (overridePage = page, overridePivot = null) => {
  const params = {
    page: overridePage,
    page_size: pageSize,
    ...
    pivot_date: overridePivot || undefined
  };
        ...
        <span style={{ margin: "0 10px" }}>
          ...
          <button onClick={() => {
            if (pageInput >= 1 && pageInput <= totalPages) {
              const pivot = data?.[data.length - 1]?.punch_date;
              fetchData(pageInput, pivot);  // 👈 pass pivot_date for fast jump
            }
          }}>Go</button>
